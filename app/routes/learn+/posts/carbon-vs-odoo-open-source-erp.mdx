...
Carbon publishes its full source on [GitHub](https://github.com/crbnos/carbon) and structures its tiers around scale and support rather than withholding core manufacturing capability. Starter includes basic ERP/MES/MRP/QMS, native accounting, and the product configurator, and Business adds the full API, workflows, custom roles and approval rules, and audit logging for a published per-user price. That's the difference between an open-core model with a paywalled ceiling on features and a source-available model with a paywall on scale and support. We unpack the distinction in [what is a source-available ERP](/learn/what-is-a-source-available-erp).
