...
- **A real, version-controlled BOM.** Not a spreadsheet copied and renamed "v3_final_FINAL" — a structured, multi-level bill of materials with revision history, so an engineering change is traceable and doesn't silently break a build in progress. See [What Is a Bill of Materials?](/learn/what-is-a-bill-of-materials) and [BOM revision control without breaking production](/learn/bom-revision-control).
- **NPI (new product introduction) support.** Tracking engineering builds, DVT/EVT/PVT stages, and the gap between an as-designed BOM and what actually got built — because early builds almost never match the CAD exactly, and someone needs a record of what changed and why.
...
The honest conclusion: legacy ERP isn't wrong, it's early. Most of these systems make more sense once a product and its BOM have actually stabilized in production — which is exactly the stage most hardware startups aren't in yet when they first go looking for "manufacturing software."

## Staged adoption: what to actually run at each phase
...
It can be, once the product and BOM have stabilized and the company has the headcount to support a real implementation — but adopting it during active NPI, when the BOM is still changing weekly, usually creates more friction than it solves.
