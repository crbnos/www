...
summary: "A practical survey of open source MES options in 2026 — what's real, what's abandoned, and the criteria (license, API, data model, self-host) that actually matter."
tag: "Developer"
author: "chase"
---

Open source MES is a smaller category than open source ERP, and searching for it turns up more forum threads asking "does this exist?" than actual production-grade software. It does exist — but the honest answer is that the field is thin, several of the well-known names are dormant or under-maintained, and most manufacturers evaluating "open source MES" are really asking a broader question: can I get execution-layer software (routings, work orders, machine data, quality holds) without being locked into a closed vendor, and can I trust it enough to run my floor on it. This article covers what MES actually is, what's genuinely available as open or source-available software in 2026, and the evaluation criteria that separate a real option from a GitHub repo that hasn't been touched in three years.

## MES, defined, before you evaluate anything

A Manufacturing Execution System sits between planning (ERP/MRP, which decides what and when to build) and the machines and people actually building it. MES tracks:
...
4. **Most "open source MES" results are actually ERP or MRP projects with an MES module bolted on**, or point tools (a barcode scanning app, a machine-monitoring dashboard) that solve one slice of execution but aren't a system of record.
...
- **Copyleft (GPL, AGPL)** — modifications you distribute must also be open. AGPL specifically closes the "SaaS loophole" — running a modified version as a hosted service still triggers the share-back requirement.
...
### 4. Can you actually self-host it, today, without a services contract
...
Check the last commit date, open issue response times, and whether releases are actually shipping. A repo with a compelling README and no commits in eighteen months is a warning sign, not a foundation.
