...
**Version it, and make the version visible on the floor.** A work instruction with no revision date is a liability during an [AS9100 or ISO audit](/learn/as9100-for-small-shops): auditors check that the instruction at the workstation matches the current released revision, not a printout from eighteen months ago.
...
- **Data capture is manual and lossy.** If the work instruction says "record the torque value," that number lives on a paper traveler until someone keys it in later, if they key it in at all. That gap is what shows up as a missing signature during an [electronic DHR review under 21 CFR Part 11](/learn/21-cfr-part-11-electronic-dhr).
...
If you want work instructions that live on the work order instead of in a binder, [try Carbon free for 30 days](/pricing) (no sales call, cancel anytime), or explore the [source on GitHub](https://github.com/crbnos/carbon) to see how routings, operations, and instructions are modeled.
