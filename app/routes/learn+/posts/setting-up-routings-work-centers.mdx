...
seoTitle: "Setting up routings and work centers"
...
The same three operations above are the exact ones used to build BRK-200's full cost, operation by operation: setup amortized over the lot, run time priced at each work center's machine-hour rate. [How to cost a machined part](/learn/how-to-cost-a-machined-part) walks that calculation through to a finished quote using this precise routing, the clearest demonstration that routing setup is the input both your quote and your schedule are built from, not a documentation exercise.
