...
## What actually breaks when BOM revision control is missing

Without a controlled process, an engineer edits the BOM in place the moment a change is approved. Here's what that does downstream:
...
- **Receiving inspection and quality** have no record of which revision a given serial number was actually built to — which matters enormously the first time a customer asks for as-built documentation or a nonconformance investigation.
...
This is the step that actually protects production. The ECO doesn't take effect the moment it's approved — it takes effect on a defined trigger:
...
Create a **new BOM revision** — never edit the old one in place. The prior revision stays exactly as it was, queryable forever, because work orders already built against it need to remain traceable to what they actually contain. The routing gets updated if the operation sequence or standard times change too (see [Setting up routings](/learn/setting-up-routings-work-centers) if you haven't formalized that side yet), and standard cost is recalculated against the new revision.
...
Once the change is fully implemented and disposition is complete, the ECO closes. The as-built record for every unit built along the way should be able to answer, permanently, which revision it was actually built to — the foundation of what we cover in [What is a digital thread?](/learn/what-is-a-digital-thread).
...
The ECO is approved with a date-effectivity of **2026-08-01**. Here's what happens to work orders around that date:
...
- **Running ECOs through email or a shared spreadsheet** instead of a system tied to the actual BOM and work order records, so the "as-built" answer is really just someone's memory.
...
A documented ECO process on paper only works if the underlying system can actually answer the query that makes the whole thing safe: "show me every open work order, and which BOM revision each one is pinned to, as of right now." That requires BOM revisions to be first-class, queryable, date-stamped records — not a single mutable BOM table that gets overwritten on every change. If you're pulling BOM data from CAD or a PLM tool into your ERP, that same versioning discipline needs to survive the sync; see [From CAD to BOM: Onshape → ERP sync](/learn/cad-to-bom-onshape-erp-sync) for how that handoff should work in practice.
...
- **Every BOM change creates a new revision with full history.** The prior revision is never overwritten, so you can always answer what a specific serial or work order was actually built against.
- **Work orders pin to the revision effective at release**, and effectivity can be set by date. Because ERP, MRP, and MES share one data model, that pin is enforced automatically — a released work order doesn't silently pick up a later change.
