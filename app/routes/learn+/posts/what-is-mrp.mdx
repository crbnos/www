...
summary: "What is MRP? A plain-English explanation of material requirements planning, with a full MRP calculation worked on a real multi-level BOM — gross to net, lead time, lot sizing."
tag: "Manufacturing basics"
author: "chase"
---

MRP (material requirements planning) is the calculation that turns a demand — a sales order, a forecast, a production schedule — into a time-phased plan of exactly what to buy and what to build, in what quantities, by what dates. It does this by exploding the bill of materials for every level of a product structure, netting out what you already have on hand or on order, and offsetting each requirement by lead time so the right material shows up exactly when it's needed, not before and not after.

Most articles on "what is MRP" stop at that definition. This one does the actual math, on a real multi-level BOM, so you can see exactly what an MRP engine computes and why a spreadsheet stops being sufficient the moment you have more than one level of structure.
...
From those four inputs, MRP produces, for every single item at every level of every BOM: **planned order releases** — what to release, how much, and when — for both purchased and manufactured items.
...
3. **Projected on-hand** — running balance: prior on-hand + scheduled receipts − gross requirements.
...
That's the whole engine. It's not complicated in concept — it's tedious to do by hand across dozens of levels and hundreds of items, which is exactly why it's automated.
...
On-hand of 6 nets the gross requirement of 50 down to a **net requirement of 44**. Lot-for-lot means the planned order receipt equals net requirement exactly — no rounding up. With a 1-week assembly lead time, the receipt in week 10 must be *released* in week 9. That release, 44 units of SMA-100, becomes the gross requirement driving every component below it.
...
Gross requirement = 44 × 1 = **44**, needed by week 9. On-hand is 0, so net requirement is 44, lot-for-lot receipt of 44. But the sensor has a **4-week purchase lead time** — so the PO must release in **week 5**, four weeks earlier than the bracket screws even though both ultimately support the same week-10 shipment. This is exactly why lead-time offsetting matters: without it, you'd place every purchase order on the same day and find out four weeks later that the long-lead item is going to be late.
...
That table — generated automatically, for every item, every time demand changes — is what MRP actually is. Change the sales order to 80 units, or push it two weeks earlier, and every one of those rows has to be recalculated across the entire structure. That's the operation a spreadsheet cannot keep up with once you have more than a handful of SKUs with real lead times: it's not the arithmetic that's hard, it's re-running it correctly, for everything, every time something changes.
...
Net requirement = gross requirement − (on-hand + scheduled receipts), calculated period by period. The net requirement is then rounded to the item's lot-sizing rule to become a planned order receipt, and that receipt is offset backward by lead time to determine the planned order release date.
...
Lot sizing rounds a net requirement up to a practical order quantity — a supplier minimum, a standard pack size, or a fixed batch. Skipping this step produces technically correct but practically unusable orders, like a purchase order for 37 screws when the supplier sells in boxes of 100.
