---
title: "How to Cost a Machined Part (Worked Example)"
publishedAt: "2026-07-18"
summary: "A step-by-step guide to machine shop costing: material, setup, cycle time, and overhead, worked through real numbers on an actual bracket, ending in a defensible price."
tag: "How-to"
author: "chase"
---

To cost a machined part, you add up five things: raw material (with a scrap allowance), setup time amortized over the lot, cycle time at each operation's machine-hour rate, any outside processing, and overhead — then apply margin to reach a price. That's the whole formula. What trips shops up isn't the math, it's skipping steps or blending numbers that should stay separate. Below is the full machine shop costing calculation worked on a real bracket, operation by operation, with a cost-breakdown table you can copy.
...
You can't cost a part from a drawing alone — you cost the routing: the specific sequence of operations, work centers, and standard times it takes to build it. If that routing doesn't exist yet, or exists only in an estimator's head, build it first. We cover exactly how in [Setting up routings: work centers, operations, and standard times](/learn/setting-up-routings-work-centers). Every number below comes straight from a routing built that way.

The part: **BRK-200**, a machined aluminum bracket — the same subassembly from our [bill of materials walkthrough](/learn/what-is-a-bill-of-materials) — quoted for a lot of 50. Its routing has three operations: saw the blank, mill the features on a 3-axis VMC, and deburr/inspect.
...
Notice the VMC setup dwarfs the others — that's typical. Milling setups (fixturing, tool offsets, first-article verification) usually carry most of a job's total setup cost, which is exactly why lot size matters so much for machined parts (more on that below).
...
With rates in hand, cycle cost is just run time × rate:
...
The milling operation alone accounts for nearly 80% of run cost — the single biggest lever if you're trying to reduce this part's price is cutting VMC cycle time, not haggling over the bandsaw rate.

## Step 5: Add outside processing

BRK-200 gets anodized after machining. The vendor quotes $1.85 per part for the lot, which you carry straight through as a pass-through direct cost — no markup logic needed at this stage, though some shops add a handling margin on outside processing, which is a legitimate policy choice, just be consistent about it.
...
(A 35% *markup on cost*, by contrast, would be $27.09 × 1.35 = $36.57 — a materially lower price for the same "35%." Know which one your accounting actually uses.)
...
Run the same part at a lot of 200 instead of 50. Material, run cost, and outside processing per part don't move — but setup dilutes:

$66.25 total setup ÷ 200 = **$0.33/part** (down from $1.33)

Direct cost drops to $3.57 + $0.33 + $16.81 + $1.85 = $22.56. Overhead at 15% is $3.38, total cost $25.94, and price at a 35% margin falls to **$39.91** — about 4% lower, entirely because setup got spread further. If your quoting process doesn't recompute setup allocation at every quantity break, you're either overcharging on large lots or losing money on small ones.

## Common mistakes that undercost (or overprice) a job

- **One blended shop rate for every machine.** A $95/hr mill and a $45/hr bench get treated as interchangeable, which overprices simple work and underprices complex work.
...
Tools like Paperless Parts and similar CAM-based instant quoting platforms do something genuinely useful: they recognize geometry from a 3D model and estimate cycle time automatically, which is a real advantage for a shop drowning in inbound RFQs that need a same-day turnaround. That's a legitimate strength worth taking seriously if quote volume is your bottleneck.

But the underlying math those tools run is exactly what's in this article — material, setup, cycle time at a machine-hour rate, overhead, margin. The real question isn't whether a tool can estimate cycle time faster than an estimator; it's whether that resulting quote then lives in the same system that runs the job, so the actual labor and machine time captured on the work order gets reconciled back against what you quoted. A quote that's disconnected from job execution never tells you whether you were right. We cover the full quoting workflow, including how to protect margin from RFQ to accepted quote, in [Job shop quoting: RFQ to margin-protected quote](/learn/job-shop-quoting-rfq-to-quote).

## How Carbon handles machined-part costing

Carbon builds this exact calculation directly into the routing and job record instead of a separate spreadsheet:
...
- **Standard cost from the routing, actual cost from the job.** Because Carbon is ERP, MRP, MES, and QMS on one Postgres data model, the same routing that generates a standard cost estimate also captures actual labor and machine time as the work order runs — so you can see variance, not just a guess.
...
A machine-hour rate is the fully loaded cost to run a specific work center for one hour — combining depreciation, tooling, consumables, facility overhead allocated to that machine, and loaded operator labor. Each work center should have its own rate rather than one shop-wide number.
...
There's no universal number — it depends on your overhead structure, market, and risk on the job — but the more important discipline is knowing whether your target is margin-on-price or markup-on-cost, since the two produce meaningfully different prices at the same stated percentage.
...
If your quotes are built on a spreadsheet that doesn't separate setup from cycle time, or uses one shop rate for every machine, it's worth seeing what routing-driven costing looks like end to end. [Try Carbon free for 30 days](/pricing) at https://app.carbon.ms, or read the costing and job data model yourself on [GitHub](https://github.com/crbnos/carbon).
