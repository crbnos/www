...
A machine hour rate is the cost of running one specific machine for one hour, all-in — not just labor, not just power, but depreciation, floor space, energy, maintenance, and allocated overhead divided by the hours that machine actually produces in a year. The machine hour rate calculation is: (depreciation + space cost + power cost + maintenance + labor + overhead) ÷ productive hours per year. Get the numerator components right and the denominator right, and you have a defensible number to quote from, cost a job against, and compare machines with. Get either wrong — most shops get the denominator wrong — and every quote built on it is wrong in the same direction, quietly, for years.
...
Every term on top is an *annual* cost for that specific machine. The denominator is the number of hours per year the machine is actually cutting chips — not the number of hours the building is open. That distinction is the single most common source of error, and it's covered in its own section below before the worked example, because it changes the answer more than any other input.
...
Facility cost per square foot should include rent or mortgage, property tax, building insurance, and general utilities not already captured under power below — a fully loaded occupancy cost, not just base rent.
...
Average draw should reflect real cutting load, not nameplate maximum — a spindle motor's rated power is rarely its sustained draw. If you don't have a power meter on the machine, a rough estimate at 40–60% of nameplate for typical machining work is a reasonable starting point until you can measure it.
...
Whether labor belongs in the machine hour rate depends on what the rate is for. A **machine-only rate** excludes labor and is used to compare equipment or to cost unattended run time (a machine running lights-out). A **fully burdened rate** includes the operator's loaded cost (wages plus payroll taxes and benefits) for the hours they attend the machine:

**Annual labor cost = Operator loaded hourly rate × Hours attended per year**

If one operator tends multiple machines, allocate their loaded cost across those machines by attended time, not evenly — a machine that needs constant attention shouldn't carry the same labor allocation as one that runs unattended for most of its cycle.
...
This is where machine hour rates most often go wrong. There are three different hour counts, and using the wrong one changes the rate by 30–50%:
...
The machine hour rate calculation must use **productive hours** — the hours the machine is actually cutting and earning its keep. Using available hours instead of productive hours understates the true cost per hour, because it spreads the same fixed costs over more hours than the machine actually delivers, and every quote built on that understated rate erodes margin on real jobs. A realistic utilization rate for a well-run job shop is 75–85%; for a shop still working out changeover and scheduling discipline, it can be well below that. If you don't know your actual utilization, measure it for a month before trusting a rate built on an assumed number — see [Setting up routings: work centers, operations, standard times](/learn/setting-up-routings-work-centers) for how standard times and actual run data get captured at the work-center level.
...
- **Per-machine rate.** The calculation above, run individually for each piece of equipment. Most accurate, and the only approach that correctly prices jobs that specifically require your most (or least) expensive machine. This is the level of detail [job shop quoting](/learn/job-shop-quoting-rfq-to-quote) needs to protect margin on RFQs that specify particular equipment or tolerances.

## How Carbon calculates and applies machine hour rates

Carbon computes machine hour rates as part of its native job costing, on the same data as scheduling, routings, and purchasing — not as a separate spreadsheet that has to be kept in sync:
...
- **Real utilization data**, because Carbon's scheduling and production tracking run on the same data model as costing — productive hours can be based on actual logged run time rather than an assumed utilization percentage.
...
There's no universal figure — it depends on purchase price, facility costs, local electricity rates, and utilization, all of which vary widely by region and equipment. A five-axis mill can easily run $80–150/hr fully burdened; a manual lathe with low depreciation might run $25–40/hr. Calculate your own using the formula above rather than borrowing an industry average.
...
Not necessarily. A high rate can simply reflect an expensive, capable machine (five-axis, live tooling, high accuracy) that lets you win work a cheaper machine can't do at all. The rate is a cost input to quoting, not a scorecard — what matters is whether the jobs quoted against that rate stay profitable.
