...
Carbon checks both of these boxes:

- **Carbon is API-first**, meaning that everything that can be done in the user interface can also be done via the API. Carbon also supports real-time event publishing, which can be used to trigger the edge device to perform some logic when a specific event occurs in the ERP/MES.
- **Carbon is flexible** enough to capture actuals from a production run either via submissions from human users, or via direct API upload from a machine tool. Regardless of how the data is collected, it can be used to monitor the progress of a job in real-time, or conduct analyses after a production run is complete.

If you’re interested in learning more about how Carbon can help you automatically close the loop, gather better data, and make better business decisions, feel free to [reach out to us](/contact) and we’ll schedule a call.
