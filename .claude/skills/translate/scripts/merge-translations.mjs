#!/usr/bin/env node
// Final step of /translate (www site). Reads the chunk outputs produced by the
// cheap-model subagents and writes each translation into the matching empty
// msgstr in the .po files. Deterministic: no model in the write path, only
// exact msgid match, and only empty msgstr are ever touched (existing
// translations are never overwritten). Reports matched / unmatched / missing.
...
for (const { locale, catalog } of Object.values(byPo)) {
  const { entries } = parsePo(
    readFileSync(`${LOCALES_DIR}/${locale}/${catalog}.po`, "utf8"),
  );
