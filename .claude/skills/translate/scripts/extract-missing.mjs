#!/usr/bin/env node
// Step 1 of /translate (www site). Scans every .po catalog for empty msgstr
// entries and writes deterministic, chunked translation jobs for cheap-model
// subagents to fill.
//
// Outputs (all under .ai/scratch/translate/, disposable):
...
//
// Locale scope: every locales/<code>/www.po on disk except the source "en".
import {
  existsSync,
  mkdirSync,
  readFileSync,
...
const { codes, labels } = readLocaleConfig(LOCALES_DIR, { readdirSync, existsSync });
        ...
        JSON.stringify(
          {
            ...
            items: slice.map((e) => ({
              ...
              note:
                e.comments.filter((c) => c.startsWith("#.")).join(" ").trim() ||
                undefined,
            })),
          },
          null,
